		...
		unsigned long arg, struct task_struct *p, unsigned long tls, struct task_struct *to_copy)
{
	...
	printk(KERN_ALERT"esi: %lx edi: %lx\n", task_pt_regs(to_copy)->si, task_pt_regs(to_copy)->di);
	...
	//savesegment(fs, p->thread.fsindex);
	p->thread.fsbase = p->thread.fsindex ? 0 : me->thread.fsbase;
	p->thread.es = me->thread.es;
	p->thread.ds = me->thread.ds;
	//savesegment(es, p->thread.es);
	//savesegment(ds, p->thread.ds);
	memset(p->thread.ptrace_bps, 0, sizeof(p->thread.ptrace_bps));
